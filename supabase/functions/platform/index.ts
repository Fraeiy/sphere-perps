import { verifySignedMessage } from 'npm:@unicitylabs/sphere-sdk@0.10.2';
...
const PLATFORM_PREFIX = '/functions/v1/platform';

function parsePath(req: Request): string {
  const url = new URL(req.url);
  let path = url.pathname;
  if (path.startsWith(PLATFORM_PREFIX)) path = path.slice(PLATFORM_PREFIX.length) || '/';
  return path.replace(/\/+$/, '') || '/';
}
