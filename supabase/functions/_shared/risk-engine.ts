...
export class RiskEngine {
  ...
  static calculateMetrics(params: RiskParams): RiskMetrics {
    ...
    const marginUsed =
      params.marginMode === 'ISOLATED' && params.isolatedMargin
        ? params.isolatedMargin
        : this.calculateMarginUsed(notional, params.leverage);
    const maintenanceMargin = this.calculateMaintenanceMargin(notional, params.maintenanceMarginRate);
    const unrealizedPnl = this.calculateUnrealizedPnl(
      params.side, params.size, params.entryPrice, params.markPrice,
    );
  ...
  static shouldLiquidate(side: PositionSide, markPrice: number, liquidationPrice: number): boolean {
  ...
  static validateLeverage(leverage: number, maxLeverage: number): void {
    ...
    if (!allowed.includes(leverage)) throw new Error(`Invalid leverage. Allowed: ${allowed.join(', ')}`);
    if (leverage > maxLeverage) throw new Error(`Leverage exceeds market maximum of ${maxLeverage}x`);
  }

  static validateMargin(availableBalance: number, requiredMargin: number): void {
...
export class TradeRiskScorer {
  static assess(params: {
    ...
    let score = 20;
    if (leverage >= 50 || accountRiskPct > 25) { risk = 'extreme'; score = 90; }
    else if (leverage >= 20 || accountRiskPct > 15) { risk = 'high'; score = 70; }
    else if (leverage >= 10 || accountRiskPct > 8) { risk = 'medium'; score = 45; }

    const trendDirection = change24h > 1 ? 'bullish' : change24h < -1 ? 'bearish' : 'neutral';
    const slPct = volatility === 'high' ? 0.02 : volatility === 'medium' ? 0.015 : 0.01;
    const tpPct = slPct * 2;
    const suggestedStopLoss = side === 'LONG' ? entryPrice * (1 - slPct) : entryPrice * (1 + slPct);
    const suggestedTakeProfit = side === 'LONG' ? entryPrice * (1 + tpPct) : entryPrice * (1 - tpPct);
    const suggestedLeverage = Math.min(leverage, volatility === 'high' ? 5 : volatility === 'medium' ? 10 : 20);
